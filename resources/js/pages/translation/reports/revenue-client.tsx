...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Revenue by Client', href: '/translation/reports/revenue-client' },
];
...
export default function RevenueByClient({ report, filters }: Props) {
    ...
    function handleFilter(e: React.FormEvent) {
        e.preventDefault();
        router.get('/translation/reports/revenue-client', { start_date: startDate, end_date: endDate }, { preserveState: true });
    }

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Revenue by Client" />
            <div className="flex flex-col gap-6 p-4 md:p-6 max-w-4xl mx-auto w-full">
                <div>
                    <h1 className="text-2xl font-bold tracking-tight">Revenue by Client</h1>
                    <p className="text-muted-foreground text-sm mt-1">
                        Top clients ranked by revenue for the selected period
                    </p>
                </div>
                                ...
                                <table className="w-full text-sm">
                                    ...
                                    <tbody className="divide-y">
                                        {report.rows.map((row, i) => (
                                            <tr key={row.client} className="hover:bg-muted/40 transition-colors">
