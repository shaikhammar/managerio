...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Revenue by Service Type', href: '/translation/reports/revenue-service-type' },
];
...
export default function RevenueByServiceType({ report, filters }: Props) {
    ...
    function handleFilter(e: React.FormEvent) {
        e.preventDefault();
        router.get('/translation/reports/revenue-service-type', { start_date: startDate, end_date: endDate }, { preserveState: true });
    }

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Revenue by Service Type" />
            <div className="flex flex-col gap-6 p-4 md:p-6 max-w-4xl mx-auto w-full">
                <div>
                    <h1 className="text-2xl font-bold tracking-tight">Revenue by Service Type</h1>
                    <p className="text-muted-foreground text-sm mt-1">
                        Revenue split by translation, editing, DTP and other services
                    </p>
                </div>
                                ...
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="border-b text-left">
                                            <th className="pb-3 font-medium text-muted-foreground">Service Type</th>
                                    ...
                                    <tbody className="divide-y">
                                        {report.rows.map((row) => (
                                            <tr key={row.service_type} className="hover:bg-muted/40 transition-colors">
                                                <td className="py-3 pr-4 font-medium">{row.service_type}</td>
