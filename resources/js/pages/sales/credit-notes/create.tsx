...
export default function CreditNoteForm({ customers, accounts, taxCodes, creditNote }: Props) {
    ...
    function handleSubmit(e: React.FormEvent) {
        ...
        if (isEditing) {
            put(`/sales/credit-notes/${creditNote!.id}`);
        } else {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={isEditing ? `Edit Credit Note ${creditNote!.number}` : 'New Credit Note'} />
            <div className="max-w-5xl mx-auto p-4 md:p-6">
                <form onSubmit={handleSubmit} className="space-y-6">
                    {/* Header */}
                    <Card>
                        <CardHeader>
                            <CardTitle>{isEditing ? `Edit Credit Note ${creditNote!.number}` : 'New Credit Note'}</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input id="reference" value={data.reference} onChange={(e) => setData('reference', e.target.value)} placeholder="e.g. Returned Services" />
                        ...
                        <CardContent>
                                ...
                                <table className="w-full min-w-[800px]">
                                    <thead>
                                        <tr className="border-b text-sm text-muted-foreground">
                                            <th className="text-left py-2 pr-2 w-[180px]">Revenue Account</th>
                            ...
                            <div className="flex justify-end mt-6">
                                <div className="w-64 space-y-2">
                                    ...
                                    <div className="flex justify-between text-lg font-bold border-t pt-2 text-red-600 dark:text-red-400">
                    ...
                    <div className="flex justify-end gap-3">
                        <Button variant="outline" type="button" asChild>
                            <Link href="/sales/credit-notes">Cancel</Link>
                        </Button>
                        <Button type="submit" disabled={processing}>
                            {processing ? 'Saving...' : isEditing ? 'Update Credit Note' : 'Create Credit Note'}
                        </Button>
                    </div>
                </form>
            </div>
